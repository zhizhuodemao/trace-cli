...
use super::{ScanResult, ProgressFn};
use super::chunk_scan;
use super::merge;
...
pub fn scan_unified_parallel(
    ...
    if data.len() < 10 * 1024 * 1024 || num_chunks <= 1 {
        return super::scan_unified(data, data_only, no_prune, skip_strings, progress_fn);
    }

    let scan_start = std::time::Instant::now();

    let format = super::types::TraceFormat::Unidbg;
    ...
    if total_lines > crate::core::scanner::LINE_MASK {
        anyhow::bail!(
            "文件行数 {} 超过当前支持的最大值 {}（约 5.36 亿行）。",
            total_lines,
            crate::core::scanner::LINE_MASK,
        );
    }
...
mod tests {
    ...
    fn scan_parallel_force(
        ...
        let format = crate::core::types::TraceFormat::Unidbg;
        let chunks_meta = split_into_chunks(data, num_chunks);

        let chunk_results: Vec<_> = chunks_meta
            .par_iter()
            .map(|meta| {
                crate::core::chunk_scan::scan_chunk(
                    data,
                    meta.start_byte,
                    meta.end_byte,
                    meta.start_line,
        ...
        let result = crate::core::merge::merge_all_chunks(chunk_results, format, data_only, skip_strings, None);
        Ok(result)
    }
    ...
    fn test_parallel_exact_match_basic() {
        ...
        let baseline = crate::core::scan_unified(data, false, false, true, None).unwrap();
    ...
    fn test_parallel_large_synthetic_data_only() {
        ...
        let baseline = crate::core::scan_unified(data, true, false, true, None).unwrap();
    ...
    fn test_parallel_no_prune_mode() {
        ...
        let baseline = crate::core::scan_unified(data, false, true, true, None).unwrap();
        assert!(baseline.scan_state.parsed_count > 0);
